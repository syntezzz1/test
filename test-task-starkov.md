...
2. В главном меню сверху выберите **Snapshot** > **Take...**.
...
После установки пакета система попытается автоматически запустить Nginx, однако столкнется с ошибкой, что по умолчанию 80 порт уже занят запущенным Apache. В следующем разделе мы удалим стандартный конфигурационный файл Nginx и изменим порт.
...
Ниже описан принцип работы установки ПО через пакетные менеджеры и компиляцию + установку исходного кода.
...
Когда требуемого пакета нет в репозитории или нужно использовать определенную версию, то можно скачать исходный код с официального сайта.
  
После скачивания исходного кода, нужно его скомпилировать и собрать с помощью **make**. Далее для установки ПО можно использовать команду **make install**.

> Необходимо наличие пакета make.
...
3. Убедитесь, что на сервере установлен OpenSSH. Его можно установить, например, с помощью следующей команды (для Debian):

```bash
apt install openssh-server
```

4. Откройте терминал на хостовой ОС и подключитесь к виртуальной машине через SHH с помощью следующей команды:

```bash
ssh root@<virtual_machine_ip_addr>
```

При первом подключении система попросит подтвердить ключ SSH и ввести пароль. Чтобы постоянно не вводить пароль можно сгенерировать пару ключей на хосте и прокинуть публичный ключ на виртуальную машину (см. инструкцию в интернете).
